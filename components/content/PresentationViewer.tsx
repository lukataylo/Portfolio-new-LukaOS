      ...
      {slides.map((slide, index) => (
        <div key={index} className="flex flex-col gap-4 border-b border-zinc-200 dark:border-zinc-800 pb-8 last:border-0">
          <div className="flex items-center gap-3">
             ...
             <h2 className={`font-bold uppercase tracking-wide text-black dark:text-white ${index === 0 ? 'text-2xl' : 'text-xl'}`}>{slide.title}</h2>
          </div>
          ...
          {/* Prose reads in the body face; mono is reserved for OS chrome. */}
          <p className={`text-zinc-600 dark:text-zinc-400 leading-relaxed whitespace-pre-line max-w-2xl ${index === 0 ? 'text-base' : 'text-sm'}`}>
            {slide.body}
          </p>
        </div>
      ))}
