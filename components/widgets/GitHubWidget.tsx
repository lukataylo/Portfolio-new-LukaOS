...
const getContributionColor = (count: number): string => {
  ...
  if (count === 1) return 'bg-red-200 dark:bg-red-900';
  if (count === 2) return 'bg-red-300 dark:bg-red-700';
  if (count === 3) return 'bg-red-400 dark:bg-red-600';
...
export const GitHubWidget: React.FC = () => {
  ...
  return (
    <div className="bg-white/80 dark:bg-zinc-900/80 backdrop-blur-xl rounded-2xl border border-black/5 dark:border-white/10 p-4 shadow-soft w-64">
      {/* Header */}
        ...
        {contributions.map((week, weekIndex) => (
          <div key={weekIndex} className="flex flex-col gap-0.5">
            {week.map((count, dayIndex) => (
              <div
                key={dayIndex}
                className={`w-2 h-2 rounded-sm ${getContributionColor(count)}`}
